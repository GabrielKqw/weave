...
const { shellQuoteSingle, shellQuotePowershell, toGitBashPath } = require('../core/quoting');
...
test('shellQuotePowershell escapes single quotes by doubling them', () => {
  assert.equal(shellQuotePowershell('hello'), "'hello'");
  assert.equal(shellQuotePowershell("it's a test"), "'it''s a test'");
  assert.equal(shellQuotePowershell("quote 'one' and 'two'"), "'quote ''one'' and ''two'''");
});

test('shellQuotePowershell round-trips arbitrary strings through powershell', { skip: process.platform !== 'win32' }, () => {
  const cases = [
    `simple`,
  ...
  for (const original of cases) {
    const wrapped = shellQuotePowershell(original);
    const result = spawnSync(
      'powershell.exe',
      ['-NoProfile', '-NonInteractive', '-ExecutionPolicy', 'Bypass', '-Command', `[Console]::Out.Write(${wrapped})`],
      { encoding: 'utf8' }
    );
    assert.equal(result.status, 0, `powershell failed for case: ${JSON.stringify(original)}`);
